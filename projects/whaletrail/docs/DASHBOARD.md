...
| **相似选股** | SQLite `daily_kline`。K 线 DTW 召回（默认 80）→ 池内换手 L1 + CYQ Wasserstein 重排。表可点选一只做 1v1：两根日 K 各用自己的价格轴（不双 Y 叠原始价），归一化收盘叠波形，换手 % 同轴叠，筹码相对价轴重叠。模板 K 与筹码上下排，只画对比窗口。不是交易账，不扩可交易名单。 | 无历史则提示跑 `fetch-baostock-universe.py`。无 turn 则筹码空。 |
