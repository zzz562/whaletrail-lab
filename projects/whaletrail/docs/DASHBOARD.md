...
| **相似选股** | SQLite `daily_kline`。K 线 DTW 召回（默认 80）→ 池内换手 L1 + CYQ 重排。精排默认偏筹码（量 35/筹 65）。**黄带/固定起止日都是扫描窗口**（改日期后必须再点金色按钮）。每次扫描写 `logs/similar-scan.log`。表含相关列；可搜召回池代码。1v1 点表。 | 无历史则提示跑 `fetch-baostock-universe.py`。无 turn 则筹码空。 |
