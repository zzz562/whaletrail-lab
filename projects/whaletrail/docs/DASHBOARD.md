...
| **相似选股** | SQLite `daily_kline`。先按收盘 DTW 从全市场**召回**（默认 80），再在池内按换手 L1 + 本地 CYQ Wasserstein **重排**（量/筹滑条，默认 60/40）。表含排序、Δ（相对召回名次）、K/量/筹。左模板 K+量柱、右模板筹码；对照只点选一只，不叠 Top5。不是交易账，不扩可交易名单。无换手则筹码空、重排可只走量。 | 无历史则提示跑 `fetch-baostock-universe.py`。无 turn 则筹码空。 |
