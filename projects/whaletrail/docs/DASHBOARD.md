...
| **相似选股** | SQLite `daily_kline`。K 线 DTW 召回（默认 80）→ 池内换手 L1 + CYQ Wasserstein 重排。当下往前可圈定黄带日期（看图用，召回仍是最近 N 根）；固定起止日即对比窗口。模板预览用「日 K / 筹码 / K+筹码」切换，一直留在扫描按钮上方。表可点选一只做 1v1。不是交易账，不扩可交易名单。 | 无历史则提示跑 `fetch-baostock-universe.py`。无 turn 则筹码空。 |
