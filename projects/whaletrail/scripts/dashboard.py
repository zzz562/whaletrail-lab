...
:root { --wt-bg:#0a0e17; --wt-surface:#111826; --wt-surface2:#0d1320; --wt-border:#1e2a3a;
  --wt-text:#e6edf3; --wt-muted:#8b98a9; --wt-gold:#e6b450; --wt-blue:#38bdf8;
  --wt-up:#4ade80; --wt-down:#f87171; --wt-mono:'JetBrains Mono',ui-monospace,monospace; }
html,body,.stApp { background:var(--wt-bg); color:var(--wt-text); font-family:Inter,-apple-system,sans-serif; }
...
.page-title { font-size:1.35rem; font-weight:800; margin:0 0 2px; }
...
[data-testid="stDataFrame"],[data-testid="stTable"] { border:1px solid var(--wt-border); border-radius:10px; overflow:hidden; }
...
def _pick_gold_sma(files: list[str]) -> Optional[str]:
    for name in files:
        ...
        if str(d.get("strategy", "")).lower().startswith("gold_sma") and str(d.get("symbol", "")).upper() == "GLD":
            return name
    for name in files:
        if "gold_sma" in name.lower() and "gld" in name.lower():
            return name
    return None
...
def _gold_book_section() -> None:
    ...
    if not gold_name:
        st.info("没有 GLD gold_sma 回测结果。有则显示；缺则空。运行 scripts/run-backtest.py gold_sma GLD。")
        return
    ...
    bh_ret = _series_return(bh)
    spy_ret = _series_return(spy)
    bh_dd = _series_drawdown(bh)
    _card_row([
    ...
    if bh_ret is not None and ret is not None and ret < bh_ret:
        st.caption("本结果中 gold_sma 弱于买入持有；对照意义在回撤，不在战胜基准。")
...
def _backtest_section() -> None:
    ...
    _card_row([
        ...
        {"label": "年化收益率", "value": f"{metrics.get('annual_return', 0):+.2f}%", "delta": "252 交易日基准", "delta_color": _num_color(metrics.get("annual_return", 0)), "sub": "", "accent": "#38bdf8"},
...
def _live_panel() -> None:
    ...
    if snap:
        ...
        for sym, info in snap.items():
            ...
            cards.append({"label": f"{sym} 扫描价", "value": f"${price:,.2f}" if price else "—", "delta": stal, "delta_color": _staleness_color(stal) if stal else "#8b98a9", "sub": " · ".join(bits), "accent": "#e6b450" if str(sym).upper() in ("GLD", "GC=F") else "#38bdf8"})
...
def _ashare_paper_section() -> None:
    ...
    _note("A 股 paper 按日收盘记账，不是盘中跟随。黄金矿股的跟庄标签在「跟庄复盘」，不进黄金账。")
    ...
    _card_row([
        {"label": "持仓", "value": str(len(positions)), "delta": "LONG 行", "delta_color": "#8b98a9", "sub": "", "accent": "#e6b450"},
    ...
    if positions:
        _show(_style_base(pd.DataFrame(positions).T.reset_index().rename(columns={"index": "代码"}).style.hide(axis="index")), width="stretch")
    else:
        st.caption("无持仓")
    if pending:
        _show(_style_base(pd.DataFrame(pending).T.reset_index().rename(columns={"index": "代码"}).style.hide(axis="index")), width="stretch")
    if trades:
...
def page_paper() -> None:
    ...
    _backtest_section()
    _sec("5m / live 扫描 · 观察")
    _note("5m 与 live 扫描仅观察，不作进场依据。")
    _live_panel()
    _ashare_paper_section()
...
def page_kol() -> None:
    ...
    raw = load_kol_eval_rows()
    rows = []
    for it in raw:
        acc = it.get("account") or it.get("账号") or it.get("handle") or it.get("user")
        if _kol_handle(acc).lower() not in KOL_ROSTER_SET:
            continue
        rows.append({"账号": f"@{_kol_handle(acc)}", "日期": it.get("date") or it.get("日期") or it.get("tweet_date") or it.get("created_at"), "标的": it.get("symbol") or it.get("ticker") or it.get("code") or it.get("标的"), "荐股": it.get("pick") or it.get("荐股") or it.get("summary") or it.get("text"), "对照日": it.get("eval_date") or it.get("对照日") or it.get("as_of"), "事后结果": it.get("outcome") or it.get("事后结果") or it.get("result") or it.get("return")})
    if not rows:
        _show(pd.DataFrame(columns=cols))
        st.info("没有已存储的评测数据。空表如上，不编对照。")
        return
    _show(_style_base(pd.DataFrame(rows, columns=cols).style.hide(axis="index")).format(na_rep="—"), width="stretch")
    st.caption(f"{len(rows)} 行来自 results/ 或 SQLite 已存评测，未请求 X。仅名册内账号。")

def page_genzhuang() -> None:
    _page_header("跟庄复盘", "watchlist 标签 观察 / 接近 / 触发 · 已收盘日 K · 不是当日阴")
    _note("跟庄只表示现有 watchlist 上的三个标签。日线已收盘 K（baostock），不用 tvscreener 快照冒充完成 bar，不编 OHLC，不与 KOL 混页。黄金矿股留在本页，不进黄金账。")
    rows = load_completed_watchlist_bars()
    if not rows:
        st.info("没有 A 股 watchlist 或无法读取。不编名单。")
        return
...
def _health_strip() -> None:
    with st.expander("健康 / 新鲜度", expanded=False):
        ...
        ashare = load_ashare_paper()
        fresh.append({"数据": "A股 paper", "时间": "有" if ashare else "无", "距今": "—"})
        ...
        qts = latest_quote_ts()
        fresh.append({"数据": "tvscreener 快照", "时间": qts or "无", "距今": _staleness(qts) if qts else "—"})
        ...
        runs = _runs_count()
        st.caption(f"SQLite runs: {runs}" if runs >= 0 else "SQLite 不可用")
